    ...
    return (
      <AnimatedSection className="contact-section">
        ...
        <div className="contact-links">
          ...
          <a href={`https://www.instagram.com/${data.instagram.slice(1)}`} target="_blank" rel="noopener noreferrer">
            {data.instagram} - Instagram
          </a>
          {section === 'personal' ? (
            <a href={`https://wa.me/${data.whatsapp.replace(/\s+/g, '')}`} target="_blank" rel="noopener noreferrer">
              {data.whatsapp} - WhatsApp
            </a>
          ) : (
            <a href={`tel:${data.phone.replace(/\s+/g, '')}`} target="_blank" rel="noopener noreferrer">
              {data.phone} - Teléfono
            </a>
          )}
        </div>
